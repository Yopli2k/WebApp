...
{
    protected const END_DATE_ID = 'end';
    protected const SELECT_ID = 'period';
    protected const START_DATE_ID = 'start';
    ...
    public function __construct(string $key, string $field, string $label)
    {
        ...
        $values = PeriodTools::getFilterOptions();
     ...
     * @return mixed
     */
    public function getValue(string $option = self::SELECT_ID): mixed
    {
        return match ($option) {
            self::START_DATE_ID => $this->startDate->getValue(),
            self::END_DATE_ID => $this->endDate->getValue(),
            default => $this->select->getValue(),
        };
     ...
     * @param string $option
     */
    public function setValue(mixed $value, string $option = self::SELECT_ID): void
    {
        switch ($option) {
     ...
     * @param string $option
     */
    private function setDateAndDisable(string $date, string $option): void
    {
     ...
     */
    private function setPeriodToDates(): void
    {
